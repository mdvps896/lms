...
class _DocumentsTabState extends State<DocumentsTab> {
      ...
      for (var material in materials) {
        if (material['files'] != null) {
          for (var file in material['files']) {
            ...
            final isVideo =
                vidExts.any((ext) => url.endsWith(ext)) || type == 'video';
            final isDoc =
                docExts.any((ext) => url.endsWith(ext)) ||
                ['pdf', 'file', 'doc', 'txt'].contains(type);
            ...
            if (isDoc && !isVideo) {
              docs.add({
                ...
                'size':
                    '${((file['size'] ?? 0) / 1024 / 1024).toStringAsFixed(1)} MB',
                'url': file['url'],
            ...
            docs
                .where(
                  (doc) =>
                      ...
                      doc['category'].toString().toLowerCase().contains(
                        widget.searchQuery.toLowerCase(),
                      ),
                )
                .toList();
  ...
  String _formatImageUrl(String url) {
    ...
    final String apiUrl =
        dotenv.env['API_URL'] ?? 'http://192.168.31.7:3000/api';
        ...
        children: [
          ...
          Expanded(
            child:
                _documents.isEmpty
                    ? ListView(
                      // Using ListView for RefreshIndicator to work on empty list
                      children: [
                        Padding(
              ...
              MaterialPageRoute(
                builder:
                    (context) => PdfViewerScreen(
                      lecture: {'title': doc['title'], 'content': doc['url']},
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: Icon(getFileIcon(), color: getFileColor(), size: 28),
            ),
